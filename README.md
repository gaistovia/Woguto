...
git init
git add .
git commit -m "Initial commit: THE PLATFORM landing site"
git branch -M main
...
| What | Where |
|---|---|
| Real signup/app URL | `script.js` → search `APP_URL` near the top. Change the one line and every "Fungua Account Bure" / "SAMBAZA ULIPWE" button uses it. |
